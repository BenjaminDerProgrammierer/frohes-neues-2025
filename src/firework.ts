...
export class Firework {
  ...
  constructor(private p: p5, color?: number, x?: number, height?: number, spread?: number) {
    this.color = color ?? p.random(255);

    x = x ?? p.random(20, p.width - 20);
    this.position = p.createVector(x, p.height);

    this.spread = spread ?? this.p.random(10, 30);
  ...
  draw() {
    ...
    if (!this.exploded) {
      this.p.stroke(this.color, 100, 100);
  ...
  explode() {
    for (let i = 0; i < 300; i++) {
      const particle = new Particle(this.p, this.position.x, this.position.y, this.color + this.p.random(-10, 10),
      this.spread);
...
class Particle {
  ...
  constructor(private p: p5, x: number, y: number, private color: number, spread: number = 30) {
  ...
  draw() {
    ...
    this.p.stroke(this.color, 100, this.lifespan);
